...
### Step 24: 스킬 이펙트와 피해 타이밍 동기화

- 스킬 버튼을 누른 즉시 피해를 예약하던 흐름을 실제 이펙트 충돌 프레임에서 예약하도록 변경했습니다.
- `PlaySkillAnimation`은 계산된 피해량을 각 스킬 코루틴에 전달하고, 스킬마다 다른 순간에 `QueueSkillImpact`를 호출합니다.
- 별 폭발은 주인공이 뛰어오른 순간, 연속 베기는 돌진이 도착한 순간, 수호의 빛은 결계가 펼쳐진 직후 피해가 발생합니다.
- 타격 순간 몬스터 체력 감소, 피격 애니메이션, 데미지 숫자, 작은 타격 파티클, 화면 흔들림이 함께 실행됩니다.
- 데미지 숫자는 별 폭발은 청록색, 연속 베기는 황금색, 수호의 빛은 녹색으로 표시해 어떤 스킬인지 구분할 수 있습니다.

#### 코드 흐름 이해하기

1. `ActivateSkill`이 스킬 종류와 피해량을 계산해 `PlaySkillAnimation`에 전달합니다.
2. 각 스킬 코루틴은 자신의 핵심 연출 프레임까지 기다립니다.
3. `QueueSkillImpact`가 해당 스킬의 종류와 피해를 전투 루프에 전달합니다.
4. 전투 루프가 같은 프레임에 체력바와 모든 피격 피드백을 갱신합니다.

#### Unity Editor에서 확인하기

1. Play 후 각 스킬을 사용하며 몬스터 체력이 버튼 클릭 즉시 줄지 않는지 확인합니다.
2. 큰 이펙트가 나타나는 순간 몬스터 피격 모션과 데미지 숫자가 함께 표시되는지 확인합니다.
3. 세 스킬의 데미지 숫자 색상이 청록색, 황금색, 녹색으로 구분되는지 확인합니다.
4. 스킬을 연속으로 예약해도 각 연출의 타격 순간에 순서대로 피해가 적용되는지 확인합니다.

### Step 25: 적 캐릭터별 발 위치 기준선

- 모든 적에게 같은 Y 좌표를 적용하던 코드를 적 캐릭터 종류에 따라 분리했습니다.
- Masked Thief는 전투 Y `-3.15`, 기본 Slime은 기존 Y `-2.75`를 사용합니다.
- 같은 Transform Y에서도 캐릭터마다 발 위치가 달라 보이는 이유는 원본 Sprite Sheet의 투명 여백과 Sprite Pivot이 서로 다르기 때문입니다.
- `monsterBattleY`에서 현재 적 종류에 맞는 기준선을 선택하고, 생성·등장·퇴장 동작은 모두 `monsterHome`을 사용하므로 위치가 일관되게 유지됩니다.

#### Unity 좌표와 Sprite Pivot 이해하기

1. Transform의 Position은 GameObject의 기준점 위치입니다.
2. Sprite Pivot은 이미지가 Transform 기준점에 붙는 위치입니다.
3. 이미지 아래쪽 투명 여백이나 Pivot이 다르면 같은 Y 좌표라도 발바닥의 화면 위치는 달라집니다.
4. 장기적으로는 모든 스프라이트의 발바닥 Pivot을 동일하게 맞추거나 캐릭터 데이터에 Y 보정값을 저장할 수 있습니다.

#### Unity Editor에서 확인하기

1. Play 후 Masked Thief의 Transform Y가 `-3.15`인지 확인합니다.
2. 등장, 공격, 피격, 퇴장 후에도 동일한 전투선으로 돌아오는지 확인합니다.
3. Slime 캐릭터를 사용하는 경우 기존 Y `-2.75`가 유지되는지 확인합니다.

### Step 26: 영웅 변경과 방치 보상 재접속 구분

- 영웅 변경 시 Scene을 다시 로드하는 동작이 실제 앱 재접속으로 처리되어 방치 보상 창이 나타나던 문제를 수정했습니다.
- 영웅 선택 직전에 `SkipOfflineRewardOnceKey`를 저장해 다음 Scene 초기화가 캐릭터 변경 때문임을 표시합니다.
- `Awake`는 이 값을 읽은 즉시 삭제하므로 오직 한 번의 초기화에만 영향을 줍니다.
- `IdleGuildProgression` 생성자에 `calculateOfflineReward` 선택값을 추가해 영웅 변경 때는 보상 계산을 건너뜁니다.
- 계산을 건너뛸 때도 `MarkExitTime`으로 기준 시간을 현재 시각으로 갱신하여 다음 실제 재접속 보상이 영웅 변경 이전 시간까지 중복 계산되지 않게 합니다.
- 앱을 종료했다 다시 실행하는 정상 흐름에는 일회성 플래그가 없으므로 기존 방치 보상이 그대로 계산됩니다.

#### Unity Editor에서 확인하기

1. Play 후 영웅 메뉴에서 소녀, 고양이, 기본 영웅을 차례로 변경합니다.
2. Scene이 다시 시작되어도 방치 보상 창이 표시되지 않는지 확인합니다.
3. 영웅 변경 후 정상적으로 자동 전투가 다시 시작되는지 확인합니다.
4. Play를 종료하고 시간이 지난 뒤 다시 실행하면 정상 방치 보상 창은 표시되는지 확인합니다.

### Step 27: 포트폴리오 전투 HUD 정보 위계

- 상단 HUD에 게임명 `IDLE GUILD`, 현재 스테이지, 선택 영웅 이름을 함께 표시하도록 정리했습니다.
- 일반 전투에서는 `NEXT BOSS 1/7` 형식으로 보스까지 남은 흐름을 보여줍니다.
- 일곱 번째 전투가 시작되면 진행 문구가 `BOSS BATTLE`로 바뀝니다.
- 전투력 옆에 `GIRL HERO`, `BLACK CAT`, `CLASSIC` 중 현재 영웅이 표시됩니다.
- 개발자 API 패널은 포트폴리오 플레이 화면을 가리지 않도록 기본 숨김 상태를 유지합니다.

#### Unity Editor에서 확인하기

1. Play 직후 상단에 게임명, 스테이지, 영웅명, 전투력이 표시되는지 확인합니다.
2. 일반 몬스터를 처치할 때 `NEXT BOSS` 숫자가 증가하는지 확인합니다.
3. 보스 등장 시 `BOSS BATTLE`로 변경되는지 확인합니다.
4. 영웅 변경 후 상단 영웅 이름도 함께 변경되는지 확인합니다.

### Step 28: 2분 포트폴리오 데모 밸런스

- 짧은 시연에서 성장, 보스, 장비, 지역 전환을 모두 볼 수 있도록 클라이언트 자동 전투 밸런스를 조정했습니다.
- 지역 전환 간격을 `stagesPerRegion = 3`으로 데이터화하여 약 2분 안에 숲·동굴·설원 변화를 확인할 수 있습니다.
- 보스 제한시간은 12초로 늘려 초기 시연에서 지나친 실패를 줄였습니다.
- 일반 골드는 기본 5, 스테이지당 3으로 올려 초반 성장 버튼을 여러 번 사용할 수 있게 했습니다.
- 보스 장비 드롭 확률은 65%로 조정해 시연 중 자동 장착이 보일 가능성을 높였습니다.
- 이 값들은 Unity 연출용 `IdleGuildBalance.asset`이며 실제 서버 경제와 권위 판정은 서버 규칙을 따로 사용합니다.

#### 2분 데모 확인 순서

1. 시작 후 일반 몬스터와 첫 보스를 처치합니다.
2. 획득 골드로 공격력·공격속도·치명타를 강화합니다.
3. 세 스킬과 전용 이펙트를 차례로 사용합니다.
4. 보스 장비 드롭과 자동 장착 안내를 확인합니다.
5. Stage 4 전후 동굴, Stage 7 전후 설원 배경과 몬스터 변경을 확인합니다.

### Step 29: 실제 서버 저장·재접속 복원 검증

- Docker Desktop에서 PostgreSQL 18과 ASP.NET Core API를 실행하고 `AddSelectedHero` 마이그레이션을 적용했습니다.
- 신규 게스트로 성장, 장비, 지역, 스킬, 선택 영웅을 저장한 뒤 API 컨테이너를 재시작해 PostgreSQL 복원을 확인했습니다.
- 검증값은 공격 4, 속도 2, 치명타 3, 장비 Tier 2/3개, 지역 1, 스킬 2/2/2, 영웅 `black_cat`입니다.
- Unity API DTO와 클라이언트에 선택 영웅 저장 및 방치 보상 미리보기 API를 연결했습니다.
- 서버 상태의 `selectedHeroId`가 로컬 선택과 다르면 서버 값을 적용하고 Scene을 한 번 다시 구성합니다.
- 영웅 선택 시 토큰이 있으면 서버 저장 성공 후에만 로컬 영웅을 변경합니다.
- 재접속 성공 후 서버 UTC 기준 방치 시간과 예상 골드를 조회해 개발 로그에 남깁니다.

### Step 30: 전투선 재조정과 지역 몬스터 공격 Sprite

- 주인공의 공통 전투 Y를 `-3.4`로 변경했습니다.
- 설원 늑대는 지역 몬스터 중 별도 기준선을 사용해 Y `-3.4`에 배치됩니다.
- 고블린, 박쥐, 늑대, 보스 악마의 외형을 유지한 공격 자세 전용 2×2 Sprite Sheet를 추가했습니다.
- 지역 몬스터는 Animator가 없는 정적 Sprite 구조이므로 적 공격 시작 시 공격 Sprite로 교체하고, 돌진·피격이 끝나면 기본 Sprite로 복원합니다.
- Masked Thief와 Slime은 기존 Animator의 Attack 상태를 계속 사용합니다.

#### 공격 Sprite 흐름

1. 지역과 보스 여부에 따라 `currentRegionMonsterIndex`를 저장합니다.
2. `EnemyAttackPattern` 시작 시 같은 인덱스의 `regionMonsterAttackSprites`를 표시합니다.
3. 몬스터가 주인공 방향으로 돌진하고 데미지를 적용합니다.
4. 원래 위치로 복귀한 뒤 `regionMonsterSprites`의 기본 자세로 되돌립니다.

#### Unity Editor에서 확인하기

1. 주인공 Transform Y가 `-3.4`인지 확인합니다.
2. 설원 늑대의 전투 Y도 `-3.4`인지 확인합니다.
3. 고블린은 곤봉 휘두르기, 박쥐는 비행 돌진, 늑대는 앞발 점프, 보스는 주먹 공격 자세가 보이는지 확인합니다.
4. 공격 후 각 몬스터가 기본 자세로 정상 복귀하는지 확인합니다.

### Step 31: 주인공 전투선과 공격 Sprite 피벗 보정

- 주인공의 공통 전투 Y를 `-3.0`으로 변경했습니다.
- 적의 실제 Transform은 공격 중 X축으로만 왕복하지만, 기본 Sprite와 공격 Sprite의 잘린 영역이 달라 교체 순간 아래로 움직이는 것처럼 보였습니다.
- 공격 Sprite 네 장을 기본 지역 몬스터와 동일한 `627×627` 셀로 통일하고 하단 중앙 피벗을 유지했습니다.
- 따라서 공격 자세로 바뀌어도 셀의 바닥 기준선이 유지되며, 공격 후 원래 Sprite로 자연스럽게 복귀합니다.

#### Unity Editor에서 확인하기

1. 주인공 Transform Y가 `-3.0`인지 확인합니다.
2. 지역 몬스터가 공격 자세로 바뀌는 순간 발의 기준선이 아래로 튀지 않는지 확인합니다.
3. 공격 돌진은 좌우로만 진행되고 복귀 후 원래 위치가 유지되는지 확인합니다.

### Step 32: 몬스터별 발 기준 전투선

- 주인공의 발선에 맞춰 보스 몬스터의 전투 Y를 `-3.6`으로 적용했습니다.
- Sprite마다 셀 내부의 투명 여백과 발바닥 픽셀 위치가 달라 모든 적에게 같은 Transform Y를 사용하면 시각적인 발선이 달라집니다.
- `GetRegionMonsterBattleY`에서 지역 몬스터별 전투선을 결정하도록 한곳에 모았습니다.
- 늑대는 `-3.4`, 보스는 `-3.6`을 사용하며, 고블린과 박쥐는 기존 일반 몬스터 기준선을 사용합니다.
- Masked Thief `-3.15`와 Slime `-2.75` 보정도 기존대로 유지됩니다.

#### Unity Editor에서 확인하기

1. 보스가 등장했을 때 Transform Y가 `-3.6`인지 확인합니다.
2. 고블린·박쥐·늑대·보스가 등장할 때 주인공의 발선과 시각적으로 같은 산길 위에 서는지 확인합니다.
3. 공격 전후 몬스터의 Y가 바뀌지 않고 X축으로만 왕복하는지 확인합니다.

### Step 33: 적 접근 거리 조정

- 지역 몬스터가 등장할 때 주인공 위치를 기준으로 계산하던 가까운 대기 지점을 제거했습니다.
- 보스는 등장 연출이 끝났을 때 X `-0.9`에서 멈추도록 고정했습니다.
- 일반 몬스터는 X `-0.55`에서 대기해 주인공과 충분한 거리를 유지합니다.
- 실제 공격 때만 보스는 왼쪽으로 `0.52`, 일반 몬스터는 `0.32`만큼 짧게 돌진한 뒤 각자의 대기 위치로 복귀합니다.

#### Unity Editor에서 확인하기

1. 보스 등장 직후 Transform X가 `-0.9`인지 확인합니다.
2. 일반 몬스터 등장 직후 Transform X가 `-0.55`인지 확인합니다.
3. 공격 중에도 적이 주인공과 겹치지 않고, 공격 후 정확한 대기 위치로 돌아오는지 확인합니다.

### Step 34: 지역 몬스터 걷기·달리기 모션

- 고블린·박쥐·늑대·보스의 외형을 유지한 달리기 자세 두 장씩을 새 4×2 Sprite Sheet로 추가했습니다.
- 고블린은 팔과 다리, 박쥐는 날개, 늑대는 앞뒤 다리, 보스는 팔과 다리가 서로 교차합니다.
- `PlayRegionMonsterRun` 코루틴이 각 몬스터의 두 프레임을 `0.11`초 간격으로 반복합니다.
- 몬스터가 등장해 달려오는 동안과 전투 대기 중에 이동 모션을 유지합니다.
- 적 공격이 시작되면 달리기 반복을 잠시 멈추고 공격 Sprite를 표시한 뒤, 복귀하면 다시 달리기 모션을 시작합니다.

#### Unity Editor에서 확인하기

1. 고블린의 양팔·양다리가 두 프레임에서 교차하는지 확인합니다.
2. 박쥐의 날개가 위아래로 움직이고 늑대가 네 다리를 교차하는지 확인합니다.
3. 보스가 무거운 보폭으로 달려온 후에도 반복 모션을 유지하는지 확인합니다.
4. 각 몬스터가 공격 자세를 재생한 뒤 달리기 모션으로 정상 복귀하는지 확인합니다.

### Step 35: 적 이동 Y 고정과 프레임 발 피벗 보정

- `-3.15`에서 약 `-2.80`으로 변하던 값은 처치 연출이 몬스터를 Y축으로 `0.35`만큼 튕기던 코드가 원인이었습니다.
- 처치와 후퇴 연출을 X축 전용으로 변경해 몬스터 Transform Y가 전투선에서 벗어나지 않게 했습니다.
- 새 달리기 시트의 두 자세는 이미지 안쪽 아래 여백이 서로 달라 Transform이 고정돼도 위아래로 흔들려 보였습니다.
- 고블린·박쥐·늑대·보스의 정지 Sprite 발 위치를 기준으로 각 달리기 프레임의 Pivot을 개별 보정했습니다.
- 공격 Sprite 네 장도 같은 발 위치를 기준으로 Pivot을 보정해 공격 자세 전환 시 높이가 튀지 않습니다.

#### Unity Editor에서 확인하기

1. 일반 몬스터 Transform Y가 이동·공격·처치 중 지정된 전투선에서 변하지 않는지 확인합니다.
2. 달리기 두 프레임을 교체할 때 발 또는 비행 기준 높이가 흔들리지 않는지 확인합니다.
3. 공격 Sprite로 전환하고 복귀할 때 몬스터가 위아래로 튀지 않는지 확인합니다.

### Step 36: 불투명 픽셀 기반 자동 발 피벗

- 수동 여백 보정은 안티앨리어싱 가장자리와 생성 이미지의 미세한 위치 차이를 완전히 처리하지 못해 잔여 흔들림이 있었습니다.
- `CreateFootAnchoredSprite`가 각 프레임을 아래에서 위로 검사해 알파값이 있는 첫 픽셀을 실제 발바닥으로 사용합니다.
- 정지·달리기·공격 Sprite를 모두 같은 함수로 생성하므로 자세가 바뀌어도 발 피벗 기준이 달라지지 않습니다.
- 관련 Texture는 런타임 픽셀 검사를 위해 Read/Write를 활성화했습니다.

#### Unity Editor에서 확인하기

1. 몬스터 Transform Y가 고정된 상태에서 두 달리기 프레임의 발 위치가 같은지 확인합니다.
2. 달리기에서 공격으로 전환할 때 발이 위아래로 움직이지 않는지 확인합니다.
3. 고블린·박쥐·늑대·보스 모두 같은 방식으로 안정화됐는지 확인합니다.

### Step 37: 자동 피벗 기준 전투선과 등장 후 정지

- 자동 발 피벗 적용 전 보스 Y `-3.6`은 이미지 내부 아래 여백을 포함한 Transform 값이었습니다.
- 자동 피벗 적용 후에는 Transform 자체가 실제 발 위치가 되므로, 지역 몬스터 Y를 주인공 전투선 `-3.0`으로 통일했습니다.
- 고블린·박쥐·늑대·보스는 화면 오른쪽에서 대기 위치까지 달려오는 동안에만 두 프레임 달리기를 반복합니다.
- 대기 위치에 도착하면 달리기 코루틴을 중지하고 정지 Sprite로 전환합니다.
- 공격 후에도 달리기 모션을 다시 시작하지 않고 정지 자세로 복귀합니다.

#### Unity Editor에서 확인하기

1. 보스의 실제 발이 주인공과 같은 전투선에 있는지 확인합니다.
2. 몬스터가 등장 중에는 달리고, 지정된 X 위치에 도착하면 정지하는지 확인합니다.
3. 공격 후 달리기 자세가 아니라 정지 자세로 돌아오는지 확인합니다.

### Step 38: 완성 UI 기준 Pixel HUD Skin

- 사용자가 제공한 `completeUI.png`를 화면 배치 참고로, `uiImage.png`를 실제 UI 요소 원본으로 사용했습니다.
- 회색 작업 배경을 제거한 투명 파생 아틀라스 `idle-guild-ui-atlas.png`를 Resources에 추가했습니다.
- 상단 HUD, 몬스터 체력 영역, 중앙 팝업, 공통 버튼을 Sprite와 9-slice 방식으로 연결했습니다.
- 하단 성장·영웅·장비·스킬·던전·상점·설정·임무 버튼은 아이콘이 포함된 전용 Sprite를 순서대로 사용합니다.
- 원본 두 PNG는 참고·재편집을 위해 그대로 보존하고 런타임에서는 투명 파생본만 로드합니다.

#### Unity Editor에서 확인하기

1. 상단 HUD와 중앙 패널 모서리가 해상도 변화에도 찌그러지지 않는지 확인합니다.
2. 하단 메뉴 8개에 성장부터 임무까지 서로 다른 아이콘이 표시되는지 확인합니다.
3. 성장·장비·스킬 패널의 버튼이 파란 Pixel UI로 표시되고 클릭 영역이 유지되는지 확인합니다.
4. 16:9와 20:9 Game View에서 Safe Area 및 텍스트 겹침을 확인합니다.
