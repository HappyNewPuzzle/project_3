...
현재 화면은 완성된 게임 UI가 아니라 서버 기능과 게임 흐름을 검증하는 학습용 프로토타입입니다. 스테이지 도전 결과에 따라 도트 캐릭터가 이동하고 공격하지만, 실제 스프라이트 애셋과 Animator 기반 애니메이션은 다음 단계에서 발전시킵니다.
...
`IdleGuildGameWorld`는 외부 이미지 없이 16x16 `Texture2D`에 색 픽셀을 채워 임시 영웅과 슬라임 Sprite를 만듭니다. `FilterMode.Point`는 확대해도 픽셀이 흐려지지 않게 합니다.
...
다음 단계에서는 코드로 만든 임시 픽셀 그림을 실제 Unity 애셋 기반 구조로 발전시키는 것이 적절합니다.

1. 캐릭터 Sprite Sheet를 프로젝트에 추가하고 Import Settings를 배웁니다.
2. Sprite Editor에서 프레임을 나눕니다.
3. Idle, Run, Attack, Hit 애니메이션 클립을 만듭니다.
4. Animator Controller로 상태 전환을 구성합니다.
5. 현재 Coroutine 이동과 서버의 스테이지 결과를 Animator 연출에 연결합니다.
